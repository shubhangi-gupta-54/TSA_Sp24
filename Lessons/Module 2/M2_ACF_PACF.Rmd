...
colnames(raw_inflow_data)=c("Month","Year", "HP1", "HP2","HP3") #column headers
...
#parse: my_date column is a character. parsing is is converting it into a date object. this code pastes the first and second column.
#pastes data from row 1 with data from row 2 separated by a hyphen
my_date <- my(my_date)  #function my from package lubridate 
#my is month year but can use any combination of d,m,y
head(my_date)
#add date data to inflow_data 
...
library(ggplot2)
ggplot(inflow_data, aes(x=my_date))+geom_line(aes(y=HP1), colour="blue")

#adding two time series to the same plot
ggplot(inflow_data, aes(x=my_date)) + geom_line(aes(y=HP1), color="blue")+geom_line(aes(y=HP2), colour="black")
```
...
HP1_acf <- acf(ts_inflow_data)  #information on the self correlation of our TS
HP1_acf

HP1_pacf=

#Option 2: Computing ACF and PACF with package "forecast". Preferred!

#The next line will output a plot since we have plot=TRUE and also store the acf values in the object
HP1_acf= Acf(ts_inflow_data, lag.max = 40) #correlation of current date with 6 months, 12 months, 18 months and 24 months)
