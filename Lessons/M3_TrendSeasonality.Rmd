...
#1+i is the index of the column data to avoid using the column names everything. i is the column name and we need to do the +1 because the first column is the date and so we want to start from the second which is then the first data column.
#need to use the print function if you use a for loop in ggplot.

#results: only the up movements - not seasonal! 
```
...
inflow_data <- inflow_data[1:(nobs-12),] #nobs is the number of observations - removes the last 12 rows.
#If there are 0s in your dataset in place of blanks, tit's going to mess up your mean so need to remove the 0s. And since you need the same number of observations in each year, remove the entire row that has a 0
